...
public interface SummaryMatchingRepository extends JpaRepository<SummaryMatching, Integer> {
    ...
    @Modifying
    @Query(value = "UPDATE cd.summary_matching SET status = 'matched',end_matching = current_timestamp,count_positive_sdn = :count_positive_sdn,count_positive_consolidate = :count_positive_consolidate,count_potential_sdn = :count_potential_sdn,count_potential_consolidate=:count_potential_consolidate " +
    ...
    @Query(value = "SELECT * FROM cd.summary_matching where CAST(start_matching AS DATE) =  CAST(:start_matching AS DATE) AND CAST(end_matching AS DATE) =  CAST(:end_matching AS DATE)", nativeQuery = true)
