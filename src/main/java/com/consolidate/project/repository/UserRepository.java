...
public interface UserRepository extends JpaRepository<Users, Integer> {
    ...
    @Query(value = "INSERT INTO ofac.Users(user_name,user_password,role_id,user_organization,status,user_full_name,created_by,created_date,updated_by,updated_date,user_token) " +
            "VALUES(:user_name,:user_password,:role_id,:user_organization,'active',:user_full_name,:created_by,current_timestamp,:created_by,current_timestamp,:user_token)", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM ofac.Users WHERE role_id =:role_id and status <>'deleted'", nativeQuery = true)
    List<Users> getUserByRoleId(@Param("role_id") int role_id);

    @Query(value = "SELECT * FROM ofac.user_role WHERE user_id =:user_id and status <>'deleted'", nativeQuery = true)
    List<Users> getUserById(@Param("user_id") int user_id);

    @Query(value = "SELECT * FROM ofac.Users where user_name =:user_name ", nativeQuery = true)
    List<Users> loginUser(@Param("user_name") String user_name);


    @Modifying
    @Query(value = "UPDATE ofac.Users SET user_password = :user_password WHERE user_id = :user_id ", nativeQuery = true)
    ...
    @Query(value = "UPDATE ofac.Users SET user_token=:user_token " +
            "where user_name =:user_name AND user_password = :user_password", nativeQuery = true)
