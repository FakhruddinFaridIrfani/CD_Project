...
public interface UserRepository extends JpaRepository<Users, Integer> {
    ...
    @Query(value = "SELECT * FROM cd.Users WHERE " +
    ...
    @Modifying
    @Query(value = "INSERT INTO cd.Users(user_name,user_password,role_id,user_organization,status,user_full_name,created_by,created_date,updated_by,updated_date,user_token) " +
    ...
    @Modifying
    @Query(value = "UPDATE cd.Users SET role_id=:role_id,user_organization=:user_organization," +
    ...
    @Modifying
    @Query(value = "UPDATE cd.Users SET status = 'deleted',updated_by=:updated_by,updated_date=current_timestamp WHERE user_id=:user_id", nativeQuery = true)
    void deleteUser(@Param("user_id") int user_id, @Param("updated_by") String updated_by);

    @Query(value = "SELECT * FROM cd.Users WHERE lower(user_name) = lower(:user_name) AND status <> 'deleted'", nativeQuery = true)
    List<Users> getUsersByName(@Param("user_name") String user_name);

    @Query(value = "SELECT * FROM cd.Users WHERE lower(user_name) = lower(:user_name) AND user_id not in(:user_id) AND status <> 'deleted'", nativeQuery = true)
    ...
    @Modifying
    @Query(value = "UPDATE cd.Users SET user_password = crypt(:user_password, gen_salt('bf')) WHERE user_id = :user_id ", nativeQuery = true)
    void changeUsersPassword(@Param("user_id") int user_id, @Param("user_password") String newPassword);

    @Modifying
    @Query(value = "UPDATE cd.Users SET user_token=:user_token " +
            "where user_name =:user_name AND user_password = crypt(:user_password, user_password)", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM cd.Users WHERE user_token =:user_token", nativeQuery = true)
